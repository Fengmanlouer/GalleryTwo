...
public class StateTransitionAnimation extends Animation {
    ...
    //*****************子视图显示什么样的内容具体这里实现************************
    /*
    mOldScreenTexture在StateTransitionAnimation.java的构造参数中申明，
    因此我们终于搞明白了GLView首先渲染的是调用StateTransitionAnimation.java这一动画的视图时传进来的材质，
    至于具体是什么材质，1.可以从GLView.java中看对StateTransitionAnimation的调用是否为空，如果不为空，
    说明这是GLView的固有属性，所有子视图都会拥有这一属性。2.为空则看某个子视图不为空的调用，则是该子视图的扩展属性。
     */
