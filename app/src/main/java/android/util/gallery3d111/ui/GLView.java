...
public class GLView {
    ...
    //************************invalidate()在View.java中表示刷新View的意思***************************************
    // 这里表明了GLView的刷新是通过requestRender()完成的。
    // 而GLRoot是一个接口，requestRender()还没有定义，需要在具体应用中具体实现如何刷新。
    ...
    //******************render()函数，类比View.java中的onDraw()函数。即该子视图显示什么样的内容由这里实现。************************
    protected void render(GLCanvas canvas) {
        ...
        //真正实现的地方在StateTransitionAnimation.java  applyOldTexture()中
   ...
   //************在GLView.java中，mTransition的值为StateTransitionAnimation intro******************
    ...
    //***********回到render()函数，接下来重点关注的是renderChild()这一函数。********************\
    //见名知义，renderChild()函数就是用来渲染GLView的子控件（这里的子控件也是GLView），
    // 这段代码中，对Canvas作了平移操作，然后调用component.render(canvas)渲染子视图,因此实现了递归渲染所有子视图到画布上。
    protected void renderChild(GLCanvas canvas, GLView component) {
        ...
        //然后调用component.render(canvas)渲染子视图,因此实现了递归渲染所有子视图到画布上
