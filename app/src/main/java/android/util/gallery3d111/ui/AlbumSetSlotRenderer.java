...
public class AlbumSetSlotRenderer extends AbstractSlotRenderer {
    ...
    //**********************AlbumSetSlotRender::renderSlot()**********************
    @Override
    public int renderSlot(GLCanvas canvas, int index, int pass, int width, int height) {
        AlbumSetEntry entry = mDataWindow.get(index);
        int renderRequestFlags = 0;
        //画内容，entry是AlbumSetEntry类，通过mDataWindow类的索引取到的数据
    ...
    //在该方法里，数据用的是entry.labelTexture，也就是说，相册集的label转换成了bitmap，
    //存放在了labelTexture.mContentBitmap中
    //label的bitmap是通过AlbumLabelMaker生成。
