...
public class SlotView extends GLView {
    ...
    //Spec类用于指定SlotView中每个slot的大小
    //有两种方法：
    //（1）指定slotWidth和slotHeight：它们指定每个slot宽度和高度。行数和slot之间的间隙将为自动确定。
    //（2）指定行rowsLand, rowsPort, and slotGap：它们指定横向/纵向模式下的行数以及slot之间的间隙。
    // 每个slot的宽度和高度是自动确定的
    ...
    public class Layout {
        ...
        public Rect getSlotRect(int index, Rect rect) {
            int col, row;//列数，行数
            if (WIDE) {//WIDE为false，纵向模式；WIDE为true,
